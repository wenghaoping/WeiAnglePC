      ...
      save () {
        this.$refs['customData'].validate((valid) => {
          if (valid) {
            ...
            this.$http.post(this.URL.setCompScheduleOrder, {user_id: localStorage.user_id, competition_id: this.competition_id, schedule: scheData})
              .then(res => {
...
//      },
      ...
      deleteStage (item) {
        if (item.schedule_id === 0) {
          ...
          this.$http.post(this.URL.deleteCompSchedule, {
            user_id: localStorage.user_id,
