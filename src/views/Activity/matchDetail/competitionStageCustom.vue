...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      save () {
        this.$refs['customData'].validate((valid) => {
          if (valid) {
            ...
            this.$http.post(this.URL.comSetScheduleOrder, {user_id: localStorage.user_id, schedule: scheData, competition_id: this.competition_id})
              .then(res => {
      ...
      // 获取所有节点
      getAllNode () {
        this.$http.post(this.URL.comGetAllNode, {
      ...
      deleteStage (item) {
        if (item.schedule_id === 0) {
          ...
          this.$http.post(this.URL.comDeleteSchedule, {
            user_id: localStorage.user_id,
